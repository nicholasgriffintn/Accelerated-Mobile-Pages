...
//				foreach ( (array) $item as $plugin ) {
                $plugin = $item;
					...
					if ( current_user_can( 'install_plugins' ) || current_user_can( 'update_plugins' ) ) {
						...
						} elseif ( $status['status'] == 'latest_installed' && ampforwp_is_plugin_active( $status['file'] ) ) {
						 	$status['activation'] = 'activated';

						} elseif ( $status['status'] == 'update_available' && ampforwp_is_plugin_active( $status['file'] ) ) {
